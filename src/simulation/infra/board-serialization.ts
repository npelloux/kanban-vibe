...
export const BoardStateSchema = z.object({
  ...
  wipLimits: WipLimits.schema,
});

export type BoardState = z.infer<typeof BoardStateSchema>;

export function serializeBoard(board: Board): BoardState {
  return {
    ...
    cards: board.cards.map((card) => ({
      ...
      workItems: {
        red: {
          total: card.workItems.red.total,
          completed: card.workItems.red.completed,
        },
        blue: {
          total: card.workItems.blue.total,
          completed: card.workItems.blue.completed,
        },
        green: {
          total: card.workItems.green.total,
          completed: card.workItems.green.completed,
        },
      },
    ...
    wipLimits: {
      options: {
        min: board.wipLimits.options.min,
        max: board.wipLimits.options.max,
      },
      redActive: {
        min: board.wipLimits.redActive.min,
        max: board.wipLimits.redActive.max,
      },
      redFinished: {
        min: board.wipLimits.redFinished.min,
        max: board.wipLimits.redFinished.max,
      },
      blueActive: {
        min: board.wipLimits.blueActive.min,
        max: board.wipLimits.blueActive.max,
      },
      blueFinished: {
        min: board.wipLimits.blueFinished.min,
        max: board.wipLimits.blueFinished.max,
      },
...
export function deserializeBoard(state: BoardState): Board {
